...
If Shadrix tools are unavailable, stop and read [client setup](references/client-setup.md). The public recovery guide is also available at `https://shadrix.xyz/agent`, with discovery metadata at `https://shadrix.xyz/.well-known/skills.json`. Never place an API key in this skill, source files, logs, URLs, or committed client configuration.
...
When tools are available but the client setup is unclear, call `get_agent_setup` with the detected client. The result is intentionally secret-free. Ask the user for a key only when the local secret configuration is ready.
